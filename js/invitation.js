...
function normalizeElement(el) {
  ...
  return {
    ...
    // invitation-text-width-handle, which is also what makes the align buttons
    // visibly do anything.
...
function normalizeInvitation(invitation) {
  if (!invitation) return getDefaultInvitation();
  return {
    elements: Array.isArray(invitation.elements) ? invitation.elements.map(normalizeElement) : [],
    background: typeof invitation.background === 'string' ? invitation.background : '#ffffff',
  };
}
...
(async function () {
  ...
  const workspaceEl = document.querySelector('.invitation-workspace');
  const toolboxEl = document.querySelector('.invitation-toolbox');
  const sheetEl = document.getElementById('invitation-sheet');
  const sheetContentEl = document.getElementById('invitation-sheet-content');
  const bgSwatchFillEl = document.getElementById('invitation-bg-swatch-fill');

  const guideV = document.createElement('div');
  guideV.className = 'invitation-guide invitation-guide-v';
  guideV.style.left = `${SHEET_WIDTH / 2}px`;
  guideV.hidden = true;
  const guideH = document.createElement('div');
  guideH.className = 'invitation-guide invitation-guide-h';
  ...
  const addQrBtn = document.getElementById('invitation-add-qr-btn');
  const addDividerBtn = document.getElementById('invitation-add-divider-btn');
  const addIconBtn = document.getElementById('invitation-add-icon-btn');
  const addImageBtn = document.getElementById('invitation-add-image-btn');
  const downloadBtn = document.getElementById('invitation-download-btn');

  const imageMenuEl = document.getElementById('invitation-image-menu');
  const imageUploadBtn = document.getElementById('invitation-image-upload-btn');
  const imagePasteBtn = document.getElementById('invitation-image-paste-btn');
  const imageUrlInput = document.getElementById('invitation-image-url-input');
  const imageUrlBtn = document.getElementById('invitation-image-url-btn');
  const imageFileInput = document.getElementById('invitation-image-file-input');

  const toolbarEl = document.getElementById('invitation-text-toolbar');
  ...
  const dividerStyleSelect = document.getElementById('invitation-divider-style-select');
  const iconSymbolSelect = document.getElementById('invitation-icon-symbol-select');
  const qrBodySelect = document.getElementById('invitation-qr-body-select');
  const qrCornerSelect = document.getElementById('invitation-qr-corner-select');
  const sizeInput = document.getElementById('invitation-size-input');
  const dividerThicknessInput = document.getElementById('invitation-divider-thickness-input');
  const dividerOrnamentSizeInput = document.getElementById('invitation-divider-ornament-size-input');
  const colorInput = document.getElementById('invitation-color-input');
  const hexInput = document.getElementById('invitation-hex-input');
  const duplicateBtn = document.getElementById('invitation-duplicate-btn');
  const deleteBtn = document.getElementById('invitation-delete-text-btn');
  const bgColorInput = document.getElementById('invitation-bg-color-input');
  const bgHexInput = document.getElementById('invitation-bg-hex-input');
  wireColorHexPair(colorInput, hexInput);
  wireColorHexPair(bgColorInput, bgHexInput);

  let invitation = getDefaultInvitation();
  let selectedId = null;
  let saveTimer = null;
  let guestUrl = '';
  ...
  function scheduleSave() {
    ...
    saveTimer = setTimeout(() => {
      Storage.setInvitation(weddingId, invitation).catch((err) => {
        console.error('setInvitation failed', err);
  ...
  function applyBackground() {
    sheetEl.style.background = invitation.background;
    if (bgSwatchFillEl) bgSwatchFillEl.setAttribute('fill', invitation.background);
  }
  ...
  function applyTextStyle(node, el) {
    ...
    // invitation-text-width-handle — only once the box is wider than its content
    ...
    const content = node.querySelector('.invitation-text-content');
    if (!content) return;
    // body.admin-theme * matches .invitation-text-content directly (it's a `*`
  ...
  function renderQrCanvas(node, el) {
    const wrap = node.querySelector('.invitation-qr-canvas');
    if (!wrap) return;
    wrap.innerHTML = '';
  ...
  function applyDividerStyle(node, el) {
    ...
    node.querySelectorAll('.invitation-divider-line').forEach((line) => {
      line.style.height = `${el.thickness}px`;
    });
    const ornamentEl = node.querySelector('.invitation-divider-ornament');
    if (ornamentEl) {
  ...
  function applyIconStyle(node, el) {
    ...
    node.style.height = `${el.size}px`;
    node.style.color = el.color;
    const glyph = node.querySelector('.invitation-icon-glyph');
    if (glyph) {
  ...
  function applyImageStyle(node, el) {
    ...
    const img = node.querySelector('.invitation-image-img');
  ...
  function deselect() {
    selectedId = null;
    sheetContentEl.querySelectorAll('.invitation-el').forEach((n) => n.classList.remove('selected'));
  ...
  function selectElement(id) {
    ...
    sheetContentEl.querySelectorAll('.invitation-el').forEach((n) => {
    ...
    dividerThicknessInput.title = t(currentLang, 'invitationThicknessTooltip');
    dividerOrnamentSizeInput.title = t(currentLang, 'invitationOrnamentSizeTooltip');
    ...
    if (isText) {
      ...
      sizeInput.title = t(currentLang, 'invitationFontSizeTooltip');
      sizeInput.min = '8';
      sizeInput.max = '200';
      sizeInput.value = el.fontSize;
    } else if (isDivider) {
      dividerStyleSelect.value = el.style;
      sizeInput.title = t(currentLang, 'invitationWidthTooltip');
      sizeInput.min = '60';
      sizeInput.max = '320';
      ...
      sizeInput.title = t(currentLang, 'invitationSizeTooltip');
      sizeInput.min = '20';
      sizeInput.max = '200';
  ...
  function updateSelected(mutator) {
    const el = invitation.elements.find((e) => e.id === selectedId);
    if (!el) return;
    mutator(el);
  ...
  function createQrNode(el) {
    ...
    node.className = 'invitation-el invitation-qr-el';
    node.dataset.id = el.id;

    const canvasWrap = document.createElement('div');
    canvasWrap.className = 'invitation-qr-canvas';
    node.appendChild(canvasWrap);

    const handle = document.createElement('span');
    handle.className = 'invitation-text-drag-handle';
    handle.innerHTML = DRAG_ICON;
    node.appendChild(handle);

    const resizeHandle = document.createElement('span');
    resizeHandle.className = 'invitation-resize-handle';
    node.appendChild(resizeHandle);

    const rotateHandle = document.createElement('span');
    rotateHandle.className = 'invitation-rotate-handle';
  ...
  function createDividerNode(el) {
    const node = document.createElement('div');
    node.className = 'invitation-el invitation-divider-el';
    node.dataset.id = el.id;

    const lineBefore = document.createElement('span');
    lineBefore.className = 'invitation-divider-line';
    const ornament = document.createElement('span');
    ornament.className = 'invitation-divider-ornament';
    const lineAfter = document.createElement('span');
    lineAfter.className = 'invitation-divider-line';
    ...
    const resizeHandle = document.createElement('span');
    resizeHandle.className = 'invitation-divider-resize-handle';
    node.appendChild(resizeHandle);
  ...
  function createIconNode(el) {
    const node = document.createElement('div');
    node.className = 'invitation-el invitation-icon-el';
    node.dataset.id = el.id;

    const glyph = document.createElement('span');
    glyph.className = 'invitation-icon-glyph';
    ...
    wireResize(resizeHandle, node, el, {
      ...
      onChange: (n, e) => {
        n.style.width = `${e.size}px`;
        n.style.height = `${e.size}px`;
        const g = n.querySelector('.invitation-icon-glyph');
  ...
  function createTextNode(el) {
    ...
    const node = document.createElement('div');
    node.className = 'invitation-el invitation-text-el';
    node.dataset.id = el.id;

    const textContentEl = document.createElement('div');
    textContentEl.className = 'invitation-text-content';
    ...
    widthHandle.className = 'invitation-text-width-handle';
    ...
    wireResize(resizeHandle, node, el, {
      ...
      min: 8,
      max: 200,
      // Must target .invitation-text-content, not the wrapper node: applyTextStyle
      ...
      onChange: (n, e) => {
        const content = n.querySelector('.invitation-text-content');
  ...
  function createImageNode(el) {
    const node = document.createElement('div');
    node.className = 'invitation-el invitation-image-el';
    node.dataset.id = el.id;

    const img = document.createElement('img');
    img.className = 'invitation-image-img';
    img.draggable = false;
    img.alt = '';
  ...
  document.addEventListener('mousedown', (e) => {
    if (e.target.closest('.invitation-el') || e.target.closest('.invitation-text-toolbar')) return;
    deselect();
  });
  ...
  addTextBtn.addEventListener('click', () => {
    const el = {
      id: generateId(),
      type: 'text',
      text: t(currentLang, 'invitationDefaultText'),
      x: 80 + (invitation.elements.length % 5) * 14,
      y: 80 + (invitation.elements.length % 5) * 28,
      ...lastTextStyle,
    };
    invitation.elements.push(el);
    const node = createTextNode(el);
    selectElement(el.id);
    const textContentEl = node.querySelector('.invitation-text-content');
    textContentEl.focus();
  ...
  addDividerBtn.addEventListener('click', () => {
    const el = {
      id: generateId(),
      type: 'divider',
      x: 60 + (invitation.elements.length % 5) * 14,
      y: 80 + (invitation.elements.length % 5) * 28,
      width: 200,
      thickness: 1,
      ornamentSize: 16,
  ...
  // invitations are stored as a Firestore field, which has a 1MB document cap,
  ...
  document.addEventListener('mousedown', (e) => {
    ...
    if (e.target.closest('.invitation-image-menu') || e.target === addImageBtn || addImageBtn.contains(e.target)) return;
    closeImageMenu();
  });
  ...
  imageFileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      console.warn('invitation image upload failed', err);
    }
  });
  ...
  imagePasteBtn.addEventListener('click', async () => {
    try {
      ...
      alert(t(currentLang, 'invitationImagePasteEmpty'));
    } catch (err) {
      alert(t(currentLang, 'invitationImagePasteUnsupported'));
    }
  });
  ...
  function addImageFromUrl(url) {
    ...
    img.onerror = () => {
      alert(t(currentLang, 'invitationImageUrlError'));
    };
    img.src = url;
  }
  ...
  document.addEventListener('paste', async (e) => {
    ...
    for (const item of items) {
      ...
      try {
        ...
        console.warn('invitation paste image failed', err);
      }
      return;
    }
  });
  ...
  function deleteSelected() {
    ...
    invitation.elements = invitation.elements.filter((e) => e.id !== selectedId);
    const node = sheetContentEl.querySelector(`[data-id="${selectedId}"]`);
    if (node) node.remove();
    deselect();
    scheduleSave();
  ...
  bgColorInput.addEventListener('input', () => {
    invitation.background = bgColorInput.value;
    applyBackground();
    scheduleSave();
  });
  ...
  function populateFontSelect() {
    ...
    fontSelect.innerHTML = `
      <optgroup label="${t(currentLang, 'invitationFontGroupHeading')}">${renderOptions(headingOptions)}</optgroup>
      <optgroup label="${t(currentLang, 'invitationFontGroupParagraph')}">${renderOptions(paragraphOptions)}</optgroup>
    `;
  }
  ...
  async function captureInvitationPng() {
  ...
  downloadBtn.addEventListener('click', async () => {
    ...
    try {
      const imgData = await captureInvitationPng();
      const { jsPDF } = window.jspdf;
      const pdf = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait' });
      const pageWidth = pdf.internal.pageSize.getWidth();
      const pageHeight = pdf.internal.pageSize.getHeight();
      pdf.addImage(imgData, 'PNG', 0, 0, pageWidth, pageHeight);
      pdf.save(`invitation-${weddingId}.pdf`);
    } finally {
  ...
  invitation = normalizeInvitation(wedding.invitation);
  applySheetSize();
  applyBackground();
  bgColorInput.value = invitation.background;
  bgHexInput.value = invitation.background.toUpperCase();
  invitation.elements.forEach((el) => createElementNode(el));
